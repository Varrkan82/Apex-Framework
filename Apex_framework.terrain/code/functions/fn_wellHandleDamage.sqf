...
if ((_entity getVariable ['QS_entity_sumDmg',0]) >= (_entity getVariable ['QS_entity_reqDmg',1500])) then {
	...
	['GRID_IG_UPDATE',['Зона Операцiй','Тунель знищено']] remoteExec ['QS_fnc_showNotification',-2,FALSE];
	if (!isNull _instigator) then {
		if (isPlayer _instigator) then {
			_instigator addScore 1;
			_text = format ['%1 (%2) знищив вхiд до ворожих тунелiв',(name _instigator),(groupID (group _instigator))];
