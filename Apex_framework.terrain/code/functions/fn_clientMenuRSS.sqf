...
_QS_ctrl_3 ctrlSetText 'Role Selection';
...
_QS_ctrl_5 ctrlSetText 'Фракцiї';
...
_QS_ctrl_7 ctrlSetText 'Опис Ролi';
...
{
	lnbAddRow [107,['',(_x # 1)]];
	if ((!(missionNamespace getVariable ['QS_RSS_client_canSideSwitch',FALSE])) && (!( (_x # 0) isEqualTo (player getVariable ['QS_unit_side',WEST])))) then {
...
_QS_ctrl_14 ctrlSetText 'Список гравцiв';
...
_QS_ctrl_17 ctrlSetTooltip 'Закрити меню обрання ролi';
...
_QS_ctrl_18 ctrlSetTooltip 'Вiдновити збереженi у профiлi налаштування до налаштувань сервера';
...
_QS_ctrl_19 ctrlSetText 'Арсенал';
_QS_buttonAction = "
	0 spawn {
		uiSleep 0.1;
		waitUntil {
...
_QS_ctrl_19 ctrlSetTooltip 'Віртуальний Арсенал';
...
_QS_buttonAction = "
	0 spawn {
		...
		50 cutText ['Використовуйте [Page Up] / [Page Down] для навігацыї по списку груп','PLAIN'];
	};
";
_QS_ctrl_20 buttonSetAction _QS_buttonAction;
_QS_ctrl_20 ctrlSetTooltip 'Налаштування Групи';
...
_QS_ctrl_23 ctrlSetTooltip 'Спробувати приєднатися до обраної ролi';
...
waitUntil {
	...
	if (!( (lnbCurSelRow 107) isEqualTo _selectedFactionRow)) then {
		missionNamespace setVariable ['QS_RSS_refreshUI',TRUE,FALSE];
	...
	if (!(_selectedFactionRow isEqualTo -1)) then {
		...
		{
			_list pushBack [
				(['GET_ROLE_DISPLAYNAME',(_x # 0)] call _fn_roles),
