...
if (_onOrOff) then {
	...
	playSound 'clickSoft';
	(missionNamespace getVariable 'QS_managed_hints') pushBack [5,FALSE,5,-1,'Трансляцiю наживо ВИМКНЕНО',[],-1];
	...
	if (!(isPipEnabled)) exitWith {
		_text = parseText 'PiP не ввiмкнено.<br/> Esc >> Configure >> Video >> PiP.';
	...
	playSound 'clickSoft';
	(missionNamespace getVariable 'QS_managed_hints') pushBack [5,FALSE,5,-1,'Трансляцiю наживо УВІМКНЕНО, зачекайте.',[],-1];
	player playAction 'Putdown';
	if (!isNil {missionNamespace getVariable 'QS_RD_CSH_TV_1'}) then {
