...
if (!(_cameraOn getVariable ['QS_carrier_launch',FALSE])) then {
	...
	if (!(_launchPos isEqualTo [])) then {
		...
		_launchCancel = {
			...
			if (!isEngineOn _cameraOn) exitWith {
				50 cutText ['Двигун має бути увiмкннено для запуску','PLAIN',0.5];
			};
			if (_cameraOn getVariable ['QS_carrier_launch',FALSE]) then {
				...
				50 cutText ['Запуск вiдмiнено','PLAIN DOWN',0.75];
			};
			_cameraOn removeAction _actionID;
			missionNamespace setVariable ['QS_client_action_carrierLaunchCancel',[],FALSE];
		};
		_launchCancelAction = _cameraOn addAction ['Вiдмiнити',_launchCancel,[_cameraOn],0,FALSE,TRUE,'','TRUE',-1,FALSE,''];
	...
	_cameraOn spawn {
		...
		if (diag_tickTime > (uiNamespace getVariable ['QS_fighterPilot_lastMsg',(diag_tickTime - 1)])) then {
			...
			[63,[4,['CAS_1',['','Близька Повiтрянна Пiдтримка доступна!']]]] remoteExec ['QS_fnc_remoteExec',-2,FALSE];
			['sideChat',[WEST,'AirBase'],(format ['Повiтрянна Пiдтримка (%1) доступна. Пiлот: %2',(getText (configFile >> 'CfgVehicles' >> (typeOf _this) >> 'displayName')),profileName])] remoteExec ['QS_fnc_remoteExecCmd',-2,FALSE];
		};
