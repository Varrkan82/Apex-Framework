...
private _RSS_MenuButton = (!((missionNamespace getVariable ['QS_missionConfig_RSS_MenuButton',0]) isEqualTo 0));
...
_QS_action_questionCivilian_text = 'Опитати цивільного';
...
_QS_action_recruit_text = 'Вербувати';
...
_QS_action_unflipVehicle_text = 'Перевернути';
...
_QS_action_commandSurrender_text = 'Наказати Капiтулювати';
...
_QS_action_secure_text = 'Знешкодити';
...
/*/===== Ear collector/*/
_QS_action_ears = nil;
_QS_action_ears_text = 'Забрати вухо';
...
_QS_action_fob_status_text = 'Статус FOB';
...
_QS_action_fob_activate_text = 'Активувати FOB';
...
_QS_action_fob_respawn_text = 'Активувати FOB респаун';
...
_QS_action_crate_customize_text = 'Редагувати Інвентар';
...
_QS_action_pushVehicle_text = 'Перевернути транспорт';
...
_QS_action_createBoat_text = 'Надути човен';
...
_QS_action_loadCargo_text = 'Завантажити вантаж';
...
_QS_action_activateVehicle_text = 'Активувати транспорт';
...
_QS_action_huronContainer_text = 'Вилiкуватись в лазареті';
...
_QS_action_sensorTarget_array = ['Повiдомити про цiль',{_this spawn (missionNamespace getVariable 'QS_fnc_clientInteractSensorTarget')},nil,60,TRUE,TRUE,'','TRUE',-1,FALSE,''];
...
if (_QS_rappelling) then {
	...
	_QS_action_rappelSelf_text = 'Швидкий спуск';
...
_QS_action_sandbagDeploy_text = 'Розгорнути стіну з мішків з піском';
_QS_action_sandbagDeploy_array = [];
_QS_interaction_sandbagDeploy = FALSE;

_QS_action_sandbagRetract = nil;
_QS_action_sandbagRetract_text = 'Розібрати';
...
if (!isNull (missionNamespace getVariable ['QS_airdefense_laptop',objNull])) then {
	...
	if (_airDefenseLaptop isEqualType objNull) then {
		if (!isNull _airDefenseLaptop) then {
			...
			_airDefenseLaptop setUserActionText [_QS_airbaseDefense_action_1,'Активувати ППО',(format ["<t size='3'>%1</t>",'Активувати ППО'])];
		};
	};
};
		...
		if (_vehicle isKindOf 'LandVehicle') then {
			if (local _vehicle) then {
				...
				if (!((missionNamespace getVariable ['QS_baseProtection_polygons',[]]) isEqualTo [])) then {
					if (!(((missionNamespace getVariable 'QS_baseProtection_polygons') findIf {(_vPos inPolygon _x)}) isEqualTo -1)) then {
...
for 'x' from 0 to 1 step 0 do {
	...
	if (_QS_module_viewSettings) then {
		if (scriptDone _deltaVD_script) then {
			if ((!(cameraOn isEqualTo _QS_cameraOn)) || {(_QS_player getVariable ['QS_RD_viewSettings_update',_false])}) then {
				...
				_fadeView = (!(cameraOn isEqualTo _QS_cameraOn));
				_QS_cameraOn = cameraOn;
				if (!isNil {_QS_player getVariable _QS_viewSettings_var}) then {
					...
					if (!isNull _QS_cameraOn) then {
						if (_QS_cameraOn isKindOf 'Man') then {
							if (!(_viewDistance isEqualTo ((_clientViewSettings # 0) # 0))) then {
								if (_fadeView) then {
							...
							if (!(_objectViewDistance isEqualTo ((_clientViewSettings # 1) # 0))) then {
								if (_fadeView) then {
							...
							if (!(_shadowDistance isEqualTo ((_clientViewSettings # 2) # 0))) then {
								if (_fadeView) then {
							...
							if ((_QS_cameraOn isKindOf 'LandVehicle') || {(_QS_cameraOn isKindOf 'Ship')}) then {
								...
								if (!(_objectViewDistance isEqualTo ((_clientViewSettings # 1) # 1))) then {
									if (_fadeView) then {
								...
								if (!(_shadowDistance isEqualTo ((_clientViewSettings # 2) # 1))) then {
									if (_fadeView) then {
								...
								if (!(_terrainGrid isEqualTo ((_clientViewSettings # 3) # 1))) then {
									setTerrainGrid ((_clientViewSettings # 3) # 1);
								};
							} else {
								if (_QS_cameraOn isKindOf 'Helicopter') then {
									if (!(_viewDistance isEqualTo ((_clientViewSettings # 0) # 2))) then {
										if (_fadeView) then {
									...
									if (!(_objectViewDistance isEqualTo ((_clientViewSettings # 1) # 2))) then {
										if (_fadeView) then {
									...
									if (!(_shadowDistance isEqualTo ((_clientViewSettings # 2) # 2))) then {
										if (_fadeView) then {
									...
									if (!(_terrainGrid isEqualTo ((_clientViewSettings # 3) # 2))) then {
										setTerrainGrid ((_clientViewSettings # 3) # 2);
									};
								} else {
									if (_QS_cameraOn isKindOf 'Plane') then {
										if (!(_viewDistance isEqualTo ((_clientViewSettings # 0) # 3))) then {
											if (_fadeView) then {
										...
										if (!(_objectViewDistance isEqualTo ((_clientViewSettings # 1) # 3))) then {
											if (_fadeView) then {
										...
										if (!(_shadowDistance isEqualTo ((_clientViewSettings # 2) # 3))) then {
											if (_fadeView) then {
										...
										if (!(_terrainGrid isEqualTo ((_clientViewSettings # 3) # 3))) then {
	...
	if (alive _QS_player) then {
		if (_lifeState in ['HEALTHY','INJURED']) then {
			...
			if (
				...
				{((!(((crew _cursorObject) findIf {(alive _x)}) isEqualTo -1)) || ((unitIsUav _cursorObject) && ([_cursorObject,0] call _fn_clientInteractUGV) && (!(((attachedObjects _cursorObject) findIf {((_x isKindOf 'CAManBase') && (alive _x))}) isEqualTo -1))))} &&
				{((!(((crew _cursorObject) findIf {(_x getVariable ['QS_RD_loaded',_false])}) isEqualTo -1)) || {(!(((crew _cursorObject) findIf {((lifeState _x) isEqualTo 'INCAPACITATED')}) isEqualTo -1))} || {(!(((attachedObjects _cursorObject) findIf {((_x isKindOf 'CAManBase') && (alive _x) && (_x getVariable ['QS_RD_loaded',_false]))}) isEqualTo -1))})}
			) then {
				{
			...
			if (
				...
				{(_cursorTarget isKindOf 'CAManBase')} &&
				{(alive _cursorTarget)} &&
				{(!(_cursorTarget isEqualTo _QS_player))} &&
			...
			if (
				...
				{(_QS_player isEqualTo (driver _QS_v2))} &&
			...
			if (
				...
				{(!((missionNamespace getVariable 'QS_module_fob_side') isEqualTo WEST))}
			) then {
			...
			if (
				...
				{(isNull (isVehicleCargo _cursorObject))} &&
				{(!( ((getPosATL _cursorObject) nearEntities [['Air','LandVehicle'],21]) isEqualTo []))}
			) then {
			...
			if (
				...
				{((!((attachedObjects _QS_player) isEqualTo [])) && (!(((attachedObjects _QS_player) findIf {([0,_x,_cursorObject] call _fn_getCustomCargoParams)}) isEqualTo -1)))}
			) then {
				{
					_object = _x;
			...
			if (
				...
				{(simulationEnabled _cursorObject)} &&
				{(!((attachedObjects _cursorObject) isEqualTo []))} &&
				{(!(((attachedObjects _cursorObject) findIf {(([0,_x,_cursorObject] call _fn_getCustomCargoParams) && (!(_x getVariable ['QS_interaction_disabled',_false])))}) isEqualTo -1))}
			) then {
			...
			//(missionNamespace getVariable ['QS_customAO_GT_active',_false])
			if (
				(_noObjectParent) &&
				{(!isNull _cursorObject)} &&
				{(_cursorObjectDistance <= 3)} &&
				{(isSimpleObject _cursorObject)} &&
				{(!((typeOf _cursorObject) isEqualTo ''))} &&
			...
			if (
				...
				{(((damage _QS_player) > 0) || {(!((((getAllHitPointsDamage _QS_player) # 2) findIf {(!(_x isEqualTo 0))}) isEqualTo -1))})}
			) then {
			...
			if ((_QS_player getUnitTrait 'uavhacker') || {(!isNull (getAssignedCuratorLogic _QS_player))}) then {
				if (
					...
					{(!((attachedObjects _QS_cO) isEqualTo []))} &&
					{(!(((attachedObjects _QS_cO) findIf {(((toLower ((getModelInfo _x) # 1)) isEqualTo _QS_action_ugv_stretcherModel) && (!(isObjectHidden _x)))}) isEqualTo -1))}
				) then {
					...
					if (!(((attachedObjects _QS_cO) findIf {((_x isKindOf 'CAManBase') && (alive _x))}) isEqualTo -1)) then {
			...
			if ((!(_QS_carrierEnabled isEqualTo 0)) && (!isNull (missionNamespace getVariable ['QS_carrierObject',_objNull]))) then {
				...
				if ((_nearSite3) || (_isNearRepairDepot2)) then {
					...
					if (_QS_action_slatArmor_vAnims isEqualTo -1) then {
						...
						if (_QS_action_slatArmor_vAnims isEqualType []) then {
							if (!(_QS_action_slatArmor_vAnims isEqualTo [])) then {
									...
									if (!(_unitName isEqualTo 'Error: No unit')) then {
										...
										if (!((toUpper _role) isEqualTo 'DRIVER')) then {
											if (!(_turretPath isEqualTo [])) then {
												_text = format ["<img image=%1/><t size='0.666'>%2</t> <t size='0.75'>%3</t><br/>",_roleImg,(_turretPath # 0),_unitName];
											} else {
	...
	if (_QS_module_safezone) then {
		if (_timeNow > _QS_module_safezone_checkDelay) then {
			_allPlayers = allPlayers;
			if (((_QS_player distance2D _QS_module_safezone_pos) < _QS_module_safezone_radius) && (!(_QS_v2 isEqualTo (missionNamespace getVariable 'QS_arty')))) then {
								...
								if ((_QS_player distance2D _QS_module_safezone_pos) > 1000) then {
									...
									if (_QS_currentTimeOnGround > _QS_maxTimeOnGround) then {
										...
										if (_QS_currentTimeOnGround > _QS_warningTimeOnGround) then {
											(missionNamespace getVariable 'QS_managed_hints') pushBack [5,_false,10,-1,'Ви пілот. Будь ласка, грайте свою роль або оберть іншу. На сервері більше 30 гравців, будь ласка, відродіться, якщо евакуація тимчасово не можливо.',[],(serverTime + 20),_true,'Role Restriction',_false];
										};
									};
								} else {
	...
	if (_QS_module_swayManager) then {
		if (_QS_uiTime > _QS_module_swayManager_checkDelay) then {
			...
			if (!(_QS_player getUnitTrait 'QS_trait_AT')) then {
				if (isNull _objectParent) then {
					if (!((secondaryWeapon _QS_player) isEqualTo '')) then {
								...
								if (!isNil {_QS_player getVariable 'QS_stamina'}) then {
									if ((_QS_player getVariable 'QS_stamina') isEqualType []) then {
										if (((_QS_player getVariable 'QS_stamina') # 1) isEqualType 0) then {
											if (!(_QS_customAimCoef isEqualTo ((_QS_player getVariable 'QS_stamina') # 1))) then {
			...
			if ((toLower (currentWeapon _QS_player)) in _QS_module_swayManager_heavyWeapons) then {
				if ((stance _QS_player) isEqualTo 'STAND') then {
					if ((!(isWeaponDeployed _QS_player)) && (!(isWeaponRested _QS_player))) then {
	...
	if (_QS_module_clientAI) then {
		if (_QS_uiTime > _QS_module_clientAI_checkDelay) then {
			if (_QS_player isEqualTo (leader (group _QS_player))) then {
				if (!(((units (group _QS_player)) findIf {((alive _x) && (!(isPlayer _x)) && (!(unitIsUav _x)))}) isEqualTo -1)) then {
											...
											if ((!(['ROBOCOP',_opsec_actionTitle,_false] call _fn_inString)) && (!(['Put Explosive Charge',_opsec_actionTitle,_false] call _fn_inString))) then {
												...
												[
													...
													[
														...
														_puid,
														2,
														(format ['Anti-Hack * Не дозволена дія в скролл-меню: "%1" %2',_opsec_actionTitle,_opsec_actionCode]),
														player,
														_QS_productVersion
													]
	...
	if (_QS_module_highCommand) then {
		if (_QS_uiTime > _QS_module_highCommand_checkDelay) then {
			...
			if (!(_QS_module_highCommand_waypoints isEqualTo [])) then {
				{
