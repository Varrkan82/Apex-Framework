...
if (!isNull _killer) then {
	if ((vehicle _killer) isKindOf 'Air') then {
		if (isPlayer (effectiveCommander _killer)) then {
			...
			if (isServer) then {
				[[WEST,'BLU'],(format ['%1 знищив %2 з %3!',_killerName,_killedDisplayName,_killerDisplayName])] remoteExec ['sideChat',-2,FALSE];
			} else {
				['sideChat',[WEST,'BLU'],(format ['%1 знищив %2 з %3!',_killerName,_killedDisplayName,_killerDisplayName])] remoteExec ['QS_fnc_remoteExecCmd',-2,FALSE];
			};
		};
	};
};
